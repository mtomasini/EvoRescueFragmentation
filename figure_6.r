# FIGURE 6
par(mar=c(5,5,2,2))
ER_data <- read.table("./Simulations/00_results_fig6.txt", header=T)
...
for (s in s.list){
    for (r in r.list) {
        for (D in D.list){
            filename <- paste("Random_SS_Island_s", s, "_r", r, "_D", D, ".eps", sep="")
            setEPS()
            postscript(filename, width=7, height=5)
            
            par1 <- (ER_data$Demes == D & ER_data$isGlobal == 0.0 & ER_data$s == s & ER_data$z == 0.02 & ER_data$r == r)
            ER1 <- ER_data[par1,]
            par2 <- (ER_data$Demes == D & ER_data$isGlobal == 1.0 & ER_data$s == s & ER_data$z == 0.02 & ER_data$r == r)
            ER2 <- ER_data[par2,]
